...
const { sendWelcomeEmail, sendFarewareEmail } = require('../email/account')
...
router.delete('/users/me', auth, async function(req, res){
    ...
    try{
        ...
        sendFarewareEmail(req.user.email, req.user.name)
        res.send(req.user)
    } catch (e) {
        res.status(500).send()
    }
...
const upload = multer({
    ...
    fileFilter(req, file, cb){
        if(!file.originalname.match(/\.(jpg|jpeg|png)$/)){
            return cb(new Error('please upload a valid picture file'))
        }
        cb(undefined, true)
    }
})
